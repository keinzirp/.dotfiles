...
- **Concise answers only**. Additional consise details may be added only when that materially helps the work.
...
- Use `jj` for version-control workflows unless the project or user specifically requires `git`.
- Use `rip` for deletion/trash workflows when available. Avoid raw destructive deletion unless explicitly requested or approved.
